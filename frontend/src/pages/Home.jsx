...
import { FiTrendingUp, FiZap, FiRefreshCw, FiAlertCircle } from 'react-icons/fi';
...
const Home = () => {
  ...
  const fetchNews = useCallback(async (cat, pg = 1, append = false) => {
    ...
    try {
      const { data } = await newsAPI.getHeadlines({ category: cat, page: pg, pageSize: 12 });
      const newArticles = data.articles || [];
      setArticles(prev => append ? [...prev, ...newArticles] : newArticles);
      setHasMore(newArticles.length === 12 && pg * 12 < (data.totalResults || 0));
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="max-w-2xl">
            ...
            <h1 className="font-display text-4xl sm:text-5xl font-bold text-ink-900 dark:text-white mb-4 leading-tight">
              News that <span className="text-gradient italic">matters</span>,<br />
              summarized by AI
            </h1>
            <p className="text-lg text-ink-600 dark:text-ink-400 leading-relaxed">
              {user ? `Welcome back, ${user.name.split(' ')[0]}! ` : ''}
              Stay informed with the latest headlines and get instant AI-powered summaries of any article.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-ink-600 dark:text-ink-400 text-center">{error}</p>
            <button onClick={() => fetchNews(activeCategory, 1)} className="btn-primary">
          ...
          <div className="text-center py-20">
            ...
            <p className="text-ink-500 dark:text-ink-400">No articles found for this category.</p>
          </div>
        ) : (
          <>
            {/* Featured Article */}
            {featured && (
              <div className="mb-8 animate-fade-in">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-semibold text-ink-600 dark:text-ink-400 uppercase tracking-wide">Featured</span>
                </div>
            ...
            {hasMore && (
              <div className="text-center mt-10">
                <button onClick={loadMore} disabled={loadingMore} className="btn-secondary gap-2">
                  {loadingMore ? (
                    <><div className="w-4 h-4 border-2 border-ink-400 border-t-transparent rounded-full animate-spin" />Loading...</>
                  ) : (
                    'Load More Articles'
                  )}
                </button>
